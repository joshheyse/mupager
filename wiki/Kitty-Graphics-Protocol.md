...
Reference for the Kitty graphics protocol as used in mupager, including the tmux-compatible Unicode placeholder method. For how mupager uses this protocol in its rendering pipeline, see [[Render Pipeline]].
...
Large images are split into 4096-byte base64 chunks. The first chunk carries all metadata; continuation chunks only need `m=`:
...
Tmux intercepts APC sequences, so they cannot reach Kitty directly. The solution has two parts: wrap graphics commands in DCS passthrough, and use Unicode placeholder characters (plain text that tmux handles natively) to mark where images appear.

### Step 1: DCS Passthrough Wrapping

Each APC sequence is wrapped individually in a tmux DCS passthrough. Every `\x1b` byte inside the sequence must be doubled:
...
Use `a=T,U=1` to transmit the image AND create a virtual placement in one command. Include `q=2` to suppress responses. Specify the placement grid size with `c=` (columns) and `r=` (rows):
...
Cell pixel dimensions come from the `TIOCGWINSZ` ioctl:
...
After the DCS-wrapped graphics commands, emit plain text that tmux passes through. Kitty recognizes this text and composites the image on top.
...
**Diacritics** — combining characters from a lookup table that encode row index, column index, and the most significant byte of the image ID. The table has 297 entries starting with `0x0305, 0x030D, 0x030E, 0x0310, ...`

**Foreground color** — encodes the lower 24 bits of the image ID using colon-separated SGR:

```
\x1b[38:2:(id>>16)&0xFF:(id>>8)&0xFF:id&0xFF m
```

**Third diacritic** — encodes `(image_id >> 24) & 0xFF`, the most significant byte of a 32-bit image ID.

**Row separators** — `\n\r` (newline then carriage return) between rows.
...
U+10EEEE RowDiac ColDiac IDDiac  ...  ← row 0
\n\r
U+10EEEE RowDiac ColDiac IDDiac  ...  ← row 1
\n\r
...
\x1b[39m                   ← reset fg
```
...
- A terminal supporting the Kitty graphics protocol (Kitty, WezTerm, Ghostty)
- For tmux: version 3.3a+ with `allow-passthrough on`
- Terminal must report pixel dimensions via `TIOCGWINSZ` for cell size calculation

See [[Requirements and Compatibility]] for full details.
